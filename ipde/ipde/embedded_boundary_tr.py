...
def get_max_R(bdy, interior=True):
    """
    Compute the maximum special coordinate width allowed
    """
    rcur = (1 if interior else -1)/bdy.curvature
    max_R = np.min(rcur[rcur > 0])
    return max_R
def get_M(bdy, h, interior=True, ratio=0.5, range=[4, 40]):
    """
    Compute the value of M to be used, for a given h (radial grid spacing)
        and "ratio" --> ratio controls the amount of max_R allowed
        and must be in (0, 1).  values near 0 will result in rapid convergence
        of the iterative solver, but higher error; larger values of ratio will
        result in better error but slower convergence
    """
    R = ratio * get_max_R(bdy, interior)
    M = int(np.ceil(R/h))
    if M < range[0]:
        return range[0]
    elif M > range[1]:
...
class EmbeddedBoundary(object):
    def __init__(self, bdy, interior, M=None, h=None, **kwargs):
        """
        ...
        if both M and h are not provided, guesses will be made
        ...
        """
        if h is None:
            h = bdy.dt*bdy.speed.min()*(2/np.pi)
        if M is None:
            M = get_M(bdy, h)
